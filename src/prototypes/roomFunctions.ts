...
Room.prototype.registerLogisticalPairs = function () {
	let energyOutboxes: Array<Id<StructureContainer>> = [];
	let energyInbox: Id<StructureContainer>;
	let sources: Array<Source> = this.find(FIND_SOURCES);
	let logisticalPairs: Array<LogisticsPair> = [];
	...
	let linkDrops: Array<StructureLink> = this.find(FIND_STRUCTURES, { filter: (i) => i.structureType == STRUCTURE_LINK && ((i.pos.x <= 4 || i.pos.x >= 45) || (i.pos.y <= 4 || i.pos.y >= 45)) });
	...
	if (energyInbox.length > 0) energyInbox = energyInboxArray[0].id;
