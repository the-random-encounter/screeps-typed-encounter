...
export function visualRCProgress(controllerID: Id<StructureController> | StructureController) {

	let lvlColor: string;

	if (typeof controllerID === 'string') controllerID = Game.getObjectById(controllerID);
	...
	if (Memory.miscData.rooms[cont.room.name].controllerPPTArray.length > 20) {
		const array: number[] = Memory.miscData.rooms[cont.room.name].controllerPPTArray;
		let sum: number = array.reduce(add, 0);
		let arrayLen: number = array.length;

		function add(accumulator: number, a: number) { return accumulator + a; }
		const avg: number = parseInt((sum / arrayLen).toFixed(2));
		Memory.miscData.rooms[cont.room.name].controllerPPTArray = [];
	...
	let sum: number = Memory.miscData.rooms[cont.room.name].controllerPPTArray.reduce(add, 0);
	let arrayLen: number = Memory.miscData.rooms[cont.room.name].controllerPPTArray.length;

	function add(accumulator: number, a: number) { return accumulator + a; }

	const avgProgressPerTick: number = parseInt((sum / arrayLen).toFixed(2));
	...
	let days: number = Math.floor(secondsRemaining / (3600 * 24));
	let hours: number = Math.floor(secondsRemaining % (3600 * 24) / 3600);
	let minutes: number = Math.floor(secondsRemaining % 3600 / 60);
	let seconds: number = Math.floor(secondsRemaining % 60);
	const alignment: alignment = cont.room.memory.settings.visualSettings.progressInfo.alignment;
	const fontSize: number = cont.room.memory.settings.visualSettings.progressInfo.fontSize;
	const stroke: string = cont.room.memory.settings.visualSettings.progressInfo.stroke;
	const xOffset: number = cont.room.memory.settings.visualSettings.progressInfo.xOffset;
	const yOffsetFactor: number = cont.room.memory.settings.visualSettings.progressInfo.yOffsetFactor;
...
export function getBody(segment: Array<BodyPartConstant>, room: Room) {
