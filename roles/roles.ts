import { repairProgress, log, returnCode } from '../prototypes/miscFunctions';
            ...
            for (let types of resTypes) {
              if (types !== RESOURCE_ENERGY)
                creep.transfer(objStorage, types)
					...
					if (room.terminal) {
						if (objTerminal.store.getUsedCapacity(RESOURCE_ENERGY) < 5000 && objStorage.store.getUsedCapacity(RESOURCE_ENERGY) > 20000) {
							creep.withdraw(objStorage, RESOURCE_ENERGY);
							cMem.termXfer = true;
							return;
						}
